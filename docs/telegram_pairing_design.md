...
sequenceDiagram
    ...
    participant Web as Nuxt 3 Portal
    ...
    Note over API: Verifies code, maps "telegram_chat_id"<br/>to User in Database.
...
We will need to:
1. Add a `telegram_chat_id` column to the `users` table to persist the linked account.
2. Create a temporary pairing codes table (`telegram_pairing_codes`) to hold active codes.

### Migration SQL
```sql
-- 1. Add column to users
ALTER TABLE users ADD COLUMN telegram_chat_id VARCHAR(50) UNIQUE NULL;
...
We will add two new endpoints in `backend/routers/auth.py` and update the booking endpoint in `backend/routers/visits.py`.

### A. Endpoint 1: Generate Code (Called by Web Frontend)
Generates a random 6-digit code, stores it in the database with a 10-minute expiration, and returns it to the client.
...
from utils.auth import get_current_user # Assuming auth helper exists

router = APIRouter(prefix="/auth/telegram", tags=["Telegram Auth"])

@router.post("/generate-code", response_model=schemas.TelegramCodeResponse)
def generate_pairing_code(
    current_user: models.User = Depends(get_current_user),
    db: Session = Depends(database.get_db)
):
    # 1. Clean up old codes for this user
    db.query(models.TelegramPairingCode).filter(
        models.TelegramPairingCode.user_id == current_user.id
    ).delete()
    ...
    # 3. Save to database
    pairing_record = models.TelegramPairingCode(
...
Verifies the pairing code received from Telegram. If valid, links the `telegram_chat_id` directly to that user's record.

```python
@router.post("/pair", response_model=schemas.TelegramPairingSuccessResponse)
def pair_telegram_account(
    payload: schemas.TelegramPairRequest,
    db: Session = Depends(database.get_db)
):
    # 1. Find the code
    pairing = db.query(models.TelegramPairingCode).filter(
        models.TelegramPairingCode.code == payload.code
    ).first()
    ...
    # 2. Check expiration
    if pairing.expires_at < datetime.datetime.now(datetime.timezone.utc):
        db.delete(pairing)
        db.commit()
        raise HTTPException(status_code=400, detail="Pairing code has expired.")

    # 3. Save the Telegram ID to the User
    user = db.query(models.User).filter(models.User.id == pairing.user_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found.")

    user.telegram_chat_id = payload.telegram_chat_id
    
    # 4. Clean up the pairing record
    db.delete(pairing)
    db.commit()

    return {"status": "success", "user_name": user.full_name, "email": user.email}
```
