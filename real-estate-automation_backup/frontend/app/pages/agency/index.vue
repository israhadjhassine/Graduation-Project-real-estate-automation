  ...
  <div class="bg-primary-50/20 min-h-screen py-12">
    <div class="max-w-7xl mx-auto px-6">
      ...
      <div class="flex gap-4 border-b border-primary-100 mb-8 overflow-x-auto pb-2">
        ...
        >
          <LucideMessageSquare class="w-4 h-4 inline-block mr-2" /> Inquiries & Notifications
        </button>
      </div>
          ...
          <table class="w-full text-left">
            <thead>
              <tr class="bg-primary-50">
                ...
                <th class="px-6 py-4 text-[10px] font-bold text-primary-400 uppercase tracking-widest">Manager</th>
      ...
      <div v-show="activeTab === 'inquiries'">
        ...
        <div class="grid md:grid-cols-2 gap-6">
          <div v-for="inq in inquiries" :key="inq.id" class="card-premium">
            ...
            <!-- Approval Action -->
            <div v-if="inq.subject === 'Sale Approval Request' && inq.status !== 'replied'" class="mt-4 pt-4 border-t border-primary-100">
               <button 
                 @click="approveSale(inq)" 
                 class="w-full py-2 bg-green-600 hover:bg-green-700 text-white rounded-xl text-[10px] font-bold uppercase transition-all shadow-lg shadow-green-900/10"
               >
                 Approve Sale
               </button>
            </div>
...
const activeProperties = computed(() => properties.value.filter(p => p.status !== 'sold'))
...
const toggleAgentStatus = async (agentId) => {
  try {
    await api.patch(`/admin/users/${agentId}/toggle-status`, {})
    fetchData()
  } catch (e) {
...
const approveSale = async (inq) => {
  if (confirm(`Are you sure you want to approve the sale of this property? This will mark it as sold and remove it from public listings.`)) {
    try {
      await api.post(`/agency/properties/${inq.property_id}/approve-sale`)
      // Also update inquiry status to 'replied' (archived)
      await api.put(`/agent/inquiries/${inq.id}/status?status=replied`)
      fetchData()
    } catch (e) {
      console.error("Failed to approve sale", e)
      alert("Error approving sale. Look at console.")
    }
  }
}
