              ...
              <tr v-for="prop in myProperties" :key="prop.id" class="hover:bg-primary-50/50 transition-colors">
                ...
                <td class="px-6 py-4">
                   <span :class="[
                     ...
                     prop.status === 'sold' ? 'bg-purple-100 text-purple-700' : 'bg-green-100 text-green-700'
                   ]">
                      {{ prop.status }}
                   </span>
                </td>
...
const markAsSold = async (id) => {
  if (!confirm("Are you sure you want to mark this property as SOLD? This will notify your manager.")) return
  try {
    await axios.patch(`${getApiUrl()}/properties/${id}/status`, { status: 'sold' }, {
      headers: { Authorization: `Bearer ${auth.token}` }
    })
    fetchData()
  } catch (e) {
